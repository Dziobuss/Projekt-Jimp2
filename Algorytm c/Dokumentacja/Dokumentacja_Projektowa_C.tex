...
\title{Dokumentacja Projektowa Projektu C\\ \large System wyznaczania współrzędnych węzłów grafów planarnych}
...
Zgodnie z wymaganiami projektowymi, program został w pełni zaimplementowany, przetestowany i jest gotowy do integracji z docelowym interfejsem graficznym w języku Java. Zrealizowano i przetestowano dwa algorytmy wyznaczania współrzędnych: model Fruchtermana-Reingolda oraz algorytm SMACOF.

\section{Metodologia testowania}
Aby porównać zaimplementowane algorytmy, wykorzystano dedykowane środowisko testowe oparte na skryptach w języku Python:
\begin{itemize}
    \item \textbf{Skrypt generujący (\texttt{generator.py}):} Pozwolił na masowe tworzenie grafów o różnej gęstości, liczbie węzłów oraz losowych wagach krawędzi. Umożliwiło to zbadanie stabilności programu w języku C przy dużych obciążeniach.
    \item \textbf{Skrypt wizualizacyjny (\texttt{wizualizacja.py}):} Moduł oparty na bibliotece \texttt{matplotlib}, wizualizujący wygenerowane przez program C współrzędne na płaszczyźnie, co pozwoliło na ocenę jakości układu grafu.
\end{itemize}
...
\begin{table}[H]
\centering
\resizebox{\textwidth}{!}{% <-- TUTAJ DODANO SKALOWANIE TABELI
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Algorytm} & \textbf{Mały graf (20 węzłów)} & \textbf{Średni graf (100 węzłów)} & \textbf{Duży graf (500 węzłów)} \\ \midrule
Fruchterman-Reingold & 29 ms & 428 ms & 7201 ms \\
SMACOF & 23 ms & 308 ms & 15284 ms \\
\bottomrule
\end{tabular}%
}
...
Na podstawie tabeli nr. 2 widać, że algorytm Fruchtermana-Reingolda radzi sobie gorzej z mniejszymi grafami, jednak wyprzedza algortym SMACOF przy bardzo dużej ilości węzłów.
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{fr_zdjecie}
        \caption{Wynik alg. Fruchtermana-Reingolda}
    \end{minipage}\hfill
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{smacof_zdjecie.png}
...
\begin{itemize}
    ...
    \item \textbf{SMACOF} wykazuje wyższą precyzję w odzwierciedlaniu różnic w wagach krawędzi (zachowanie proporcji metrycznych)..
\end{itemize}
