...
\begin{itemize}
    \item \textbf{\texttt{graph.h}:} Główny plik nagłówkowy spinający projekt. Zawiera definicje struktur danych (\texttt{Vertex}, \texttt{Edge}), stałych matematycznych (np. \texttt{EPSILON} dla algorytmu SMACOF) oraz deklaracje prototypów funkcji obliczeniowych.
    
    \item \textbf{\texttt{main.c}:} Moduł inicjujący i sterujący. Parsuje argumenty linii poleceń (\texttt{argc}, \texttt{argv}), wczytuje i weryfikuje dane topologii, dynamicznie alokuje pamięć, inicjalizuje wstępne współrzędne, a po wykonaniu obliczeń zapisuje wyniki do plików.
...
\subsection{Moduły testowe i narzędziowe (Python)}
...
Podstawowymi strukturami wykorzystywanymi do reprezentacji grafów (zdefiniowanymi w \texttt{graph.h}) są:
...
Poniższa specyfikacja definiuje formaty danych wymienianych między modułem obliczeniowym w języku C a aplikacją kliencką w języku Java.
...
\begin{enumerate}
    \item \textbf{Liczba wierzchołków ($N$):} Pierwsze 4 bajty pliku to zwykła liczba całkowita (typ \texttt{int}). Informuje ona program w Javie, ile łącznie węzłów zapisano w pliku, dzięki czemu wiadomo, ile razy należy wykonać pętlę wczytującą kolejne dane.
    \item \textbf{Dane pojedynczego wierzchołka:} Reszta pliku wypełniona jest blokami 20-bajtowymi. Architektura bloku to: $4$ bajty przestrzeni \texttt{id} (Signed Integer 32-bit), $8$ bajtów \texttt{x} (Double 64-bit) oraz $8$ bajtów \texttt{y} (Double 64-bit).
\end{enumerate}
